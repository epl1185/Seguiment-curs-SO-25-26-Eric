...
::: {.definition-box}
**Context**: En aplicacions crítiques per a la integritat de dades (com videojocs multijugador competitius: *League of Legends*), es requereixen mecanismes de protecció a nivell de kernel per prevenir manipulacions no autoritzades de memòria i executables.

L'objectiu és detectar i bloquejar scripts, exploits i eines de trampes que alteren el comportament del joc.
:::
...
::: {.definition-box}
**Driver / Controlador de Kernel**: Mòdul de codi amb privilegis elevats que s'executa en *kernel mode* (Ring 0) i pot interactuar directament amb el hardware i amb estructures internes del sistema operatiu.

En el cas de LoL, aquest driver és **vgc.sys** (Vanguard).
:::

::: {.definition-box}
**Callback (en kernel)**: Funció registrada dins d'una estructura del sistema operatiu que és cridada automàticament quan ocorre un determinat esdeveniment (creació de processos, accés a memòria, càrrega de mòduls, etc.).

No és simplement una funció passada com a paràmetre (com en l'espai s'usuari), sinó una funció integrada dins del flux intern del SO.
:::
...
::: {.fragment}
Per això, un sistema anti-trampa real no pot funcionar només en el **Ring 0**.
:::
...
## Callbacks en el kernel: de baix nivell a alt nivell

::: {.definition-box}
**IDT (Interrupt Descriptor Table)**: (La Interrupt Descriptor Table (**IDT**) és una estructura del kernel que indica al CPU la adreça de cada **ISR**.) (Definició anterior)
:::
...
[https://www.leagueoflegends.com/es-es/news/dev/dev-vanguard-x-lol/](https://www.leagueoflegends.com/es-es/news/dev/dev-vanguard-x-lol/)
[https://gist.github.com/stdNullPtr/2998eacb71ae925515360410af6f0a32](https://gist.github.com/stdNullPtr/2998eacb71ae925515360410af6f0a32)
:::
