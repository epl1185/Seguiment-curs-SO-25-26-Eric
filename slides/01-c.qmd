...
Es considera una matriu `m` de mida **MIDA × MIDA** on cada posició `(i,j)`
representa el resultat d’un partit entre els equips *i* i *j*:

- l’equip *i* ha marcat **x** gols  
- l’equip *j* ha marcat **y** gols  

Es vol determinar **quants partits compleixen**:
...
int generar_valor() {
    ...
    //(7) No podem utilitzar la estructura switch. Digueu el perqué.
    if (r < 0.25) return 0;
...
int checkParells(int* suma, partit** partits)
{
    ...
    for(int i = 0; i < MIDA; i++)
    {
        for(int j = 0; j < MIDA; j++)
        {
            if((partits[i][j].golsFavor + partits[i][j].golsContra) == /*(8)*/)
            {
              count++;
            }
        }
...
```
- Digueu que hauriem de canviar a nivell de sintaxi en el codi si en compte de tenir:  
```c
...
MIDA * sizeof(partit)
```
- (3): "." Ja que de l'altra manera estem dient que apuntem a un partit*.
- (4): El compilador posa valors brossa perquè no existeix cap partits[i][j] que sigui enter
- (5): atoi(argv[1])
- (6): free(partits[i])
- (7): Perquè el switch divideix per casos enters (case 0, case 1, ...) i no per numeros float.
- (8): *suma (pas de paràmetres)
